...
#Compute perCellQCMetrics on counts matrix:
perCellQCMetrics <- scater::perCellQCMetrics(assay(sce1, "counts"))
colData(sce1) <- cbind(colData(sce1), perCellQCMetrics)

#Filter cells with low column sum and create a new object:
sce1 <- sce1[, order(-sce1@colData$sum)]
print(sce1@colData$sum[1:10])
sce2 <- sce1[,1:round(ncol(sce1) * 0.40)]

#Normalize sce2 using scater library and store it back:
assay(sce2, "filteredCellsNormalized") <- scater::normalizeCounts(assay(sce2, "counts"))
print(sum(assay(sce2, "filteredCellsNormalized")))

#Find highly variable genes from the normalized assay in the previous step using scran library and make a new object:
topHVG1000 <- scran::getTopHVGs(scran::modelGeneVar(assay(sce2, "filteredCellsNormalized")), n = 1000)
sce3 <- sce2[topHVG1000,]
print(topHVG1000[1:10])

print(sum(assay(sce3, "filteredCellsNormalized")))
#Run ‘PCA’ on the highly variable genes computed in the last step using scater library:
reducedDim(sce3, type = "PCA") <- scater::calculatePCA(assay(sce3, "filteredCellsNormalized"))
print(reducedDim(sce3, type = "PCA")[1:4, 1:4])
...
#Compute perCellQCMetrics on counts matrix:
perCellQCMetrics <- scater::perCellQCMetrics(assay(es, "counts"))
colData(es) <- cbind(colData(es), perCellQCMetrics)
...
#Normalize ‘filteredCells’ subset using scater library and store it back:
es <- ExperimentSubset::setSubsetAssay(es, "filteredCells", scater::normalizeCounts(assay(es, "filteredCells")), subsetAssayName = "filteredCellsNormalized")


#Find highly variable genes from the normalized assay in the previous step using scran library against the ‘filteredCells’ subset only:
topHVG1000 <- scran::getTopHVGs(scran::modelGeneVar(assay(es, "filteredCellsNormalized")), n = 1000)
es <- createSubset(es, "hvg1000", rows = topHVG1000, parentAssay = "filteredCellsNormalized")
print(topHVG1000[1:10])
...
print(reducedDim(es, type = "PCA", subsetName = "hvg1000")[1:4, 1:4])

```

As evident from the workflow output above, the data stored, manipulated and output for a particular input data is same throughout the workflow either using the `SingleCellExperiment` object or our `ExperimentSubset` object which validates our package.
