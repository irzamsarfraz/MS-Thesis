...
As the overall purpose of the package is to save memory and provide provenance tracking in an analysis when subsets are created, we run a sample workflow from OSCA. The sample workflow is given below:

- load data
- compute qc metrics
- use colsums as qc metric to filter the cells and keep the top 40% of the cells that have the highest colsums
- normalize data
- get top 1000 variable features
- compute pca on variable features
- perform clustering on the pca dimensions of variable features
...
The sample analysis described above creates multiple subsets and as can be seen that 3 SCE objects are required to complete the analysis, therefore adding to the redundancy of the data. Now we run the same analysis using ES:
...
As can be seen from the code above, only a single ES object is used. 

To further demonstrate the effectiveness of our package, we use multiple datasets and re-run the analyses and compute the memory savings with our package. Figure below illustrates the savings managed through our package in comparison with SCE:
...
- sce will need to create 3 objects

We also show provenance tracking:
